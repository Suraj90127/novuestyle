...
class authControllers {
  ...
  shipping = async (req, res) => {
    const { shipping_fee, cod_fee } = req.body;
    try {
      const shipping = await shipingModel.findOne();
      if (shipping) {
        await shipingModel.updateOne({ shipping_fee, cod_fee });
        responseReturn(res, 201, {
          message: "Shipping fee updated successfully",
        });
      } else {
        const shipping = await shipingModel.create({ shipping_fee, cod_fee });
        responseReturn(res, 201, {
  ...
  getShipping = async (req, res) => {
    try {
      const shipping = await shipingModel.findOne();
      if (shipping) {
        responseReturn(res, 201, { shipping });
      } else {
        responseReturn(res, 201, { message: "Shipping fee not found" });
      }
    } catch (error) {
      responseReturn(res, 500, { error: "Internal server error" });
    }
