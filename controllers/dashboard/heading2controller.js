const Heading2 = require("../../models/Heading2");

// Create a new heading2
exports.createHeading = async (req, res) => {
  try {
    const { heading2, categorys } = req.body; // match frontend
    if (!heading2) {
      return res.status(400).json({ message: "heading2 is required" });
    }

    // Ensure categorys is always an array
    const formattedCategory = Array.isArray(categorys) ? categorys : [categorys];

    const newHeading = new Heading2({
      heading2,
...
exports.getHeadings = async (req, res) => {
  try {
    const headings = await Heading2.find();
...
// Get a single heading2 by ID
exports.getHeadingById = async (req, res) => {
  try {
    const heading = await Heading2.findById(req.params.id);
    if (!heading) {
...
exports.updateHeading = async (req, res) => {
  try {
    ...
    const updatedHeading = await Heading2.findByIdAndUpdate(
      req.params.id,
      { $set: updatedData },
      { new: true }
    );
...
exports.deleteHeading = async (req, res) => {
  try {
    const deletedHeading = await Heading2.findByIdAndDelete(req.params.id);
    if (!deletedHeading) {
      return res.status(404).json({ message: "Heading not found" });
    }
