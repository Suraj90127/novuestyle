...
export const get_design_get = createAsyncThunk(
    'chat/design-get',
    async (_, { rejectWithValue, fulfillWithValue }) => {
        try {
            const { data } = await api.get(`/design-get`, { withCredentials: true })
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
...
export const delete_design = createAsyncThunk(
    'chat/delete_design',
    async (receverId, { rejectWithValue, fulfillWithValue }) => {
        try {
            const { data } = await api.post(`/delete_design/${receverId}`, { withCredentials: true })
            return fulfillWithValue(data)
        } catch (error) {
            return rejectWithValue(error.response.data)
        }
...
export const chatReducer = createSlice({
    ...
    extraReducers: {
        ...
        [get_design_get.fulfilled]: (state, { payload }) => {
            state.totalDeign = payload.totaldesign
            state.designs = payload.designs
        },
        [delete_design]: (state) => {
            state.loader = true;
        },
        [delete_design.fulfilled]: (state, { payload }) => {
            state.loader = false;
            state.successMessage = payload.message;
        },
        [delete_design.rejected]: (state, { payload }) => {
            state.loader = false;
