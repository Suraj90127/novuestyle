...
export const get_seller_created_orders = createAsyncThunk(
  "order/get_seller_created_orders",
  async (
    { parPage, page, searchValue, sellerId },
    { rejectWithValue, fulfillWithValue }
  ) => {
    try {
      const { data } = await api.get(
        `/seller/create-orders?page=${page}&searchValue=${searchValue}&parPage=${parPage}`,
        { withCredentials: true }
      );
      return fulfillWithValue(data);
    } catch (error) {
...
export const seller_order_remark_update = createAsyncThunk(
  "order/seller_order_remark_update",
  async ({ orderId, info }, { rejectWithValue, fulfillWithValue }) => {
    try {
      const { data } = await api.put(
        `/seller/order-remark/update/${orderId}`,
        info,
        { withCredentials: true }
      );
      return fulfillWithValue(data);
...
export const OrderReducer = createSlice({
  ...
  extraReducers: {
    ...
    [get_seller_created_orders.fulfilled]: (state, { payload }) => {
      state.createdOrders = payload.orders;
      state.totalOrder = payload.totalOrder;
    },
    ...
    [seller_order_remark_update.rejected]: (state, { payload }) => {
      state.errorMessage = payload.message;
    },
    [seller_order_remark_update.fulfilled]: (state, { payload }) => {
      state.successMessage = payload.message;
    },
  },
});
