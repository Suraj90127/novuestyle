...
import {
  ...
  updateHeading,
  deleteHeading,
} from "../../store/Reducers/headingsReducer";
import { get_category } from "../../store/Reducers/categoryReducer";

const HeadingsManager = () => {
  const dispatch = useDispatch();
  const { headings, loading, error } = useSelector((state) => state.headings);
  ...
  const handleSubmit = (e) => {
    ...
    const data = { heading1: headingInput, categorys: categoryInput };
  ...
  const handleEdit = (heading) => {
    ...
    setHeadingInput(heading.heading1);
        ...
        <table className="min-w-full border-collapse text-gray-100">
          ...
          <tbody className="divide-y divide-gray-700">
            {headings.map((h) => (
              ...
              >
                <td className="p-3">{h.heading1}</td>
                <td className="p-3">
                  {h.categorys?.map((sub) => (
                    <div
                      key={sub._id}
...
export default HeadingsManager;
