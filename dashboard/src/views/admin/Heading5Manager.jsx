...
import {
  ...
  updateHeading,
  deleteHeading,
} from "../../store/Reducers/heading5Reducer";
import { get_category } from "../../store/Reducers/categoryReducer";

const Heading5Manager = () => {
  const dispatch = useDispatch();
  const { headings, loading, error } = useSelector((state) => state.heading5);
  ...
  const handleSubmit = (e) => {
    ...
    const data = { heading5: headingInput, categorys: categoryInput };
  ...
  const handleEdit = (heading) => {
    ...
    setHeadingInput(heading.heading5);
              ...
              {loader ? (
                ...
                categorys.map((cat) =>
                  cat.subCategory.map((sub) => {
                    const selected = categoryInput.some(
                      (c) => c._id === sub._id
                    );
                    return (
                      <div
        ...
        <table className="min-w-full border-collapse text-gray-100">
          ...
          <tbody className="divide-y divide-gray-700">
            {headings.map((h) => (
              ...
              >
                <td className="p-3">{h.heading5}</td>
                <td className="p-3">
                  {h.categorys?.map((sub) => (
                    <div
                      key={sub._id}
...
export default Heading5Manager;
