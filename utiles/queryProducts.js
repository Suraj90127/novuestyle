class queryProducts {
  ...
  searchQuery = () => {
    if (!this.query.searchValue) return this;
    const sv = this.query.searchValue.toLowerCase();
    this.products = this.products.filter(
      (p) =>
        (p.name && p.name.toLowerCase().includes(sv)) ||
        (p.category && p.category.toLowerCase().includes(sv)) ||
        (p.subCategory && p.subCategory.toLowerCase().includes(sv)) ||
        (p.description && p.description.toLowerCase().includes(sv))
    );
    return this;
  };
  sortByPrice = () => {
