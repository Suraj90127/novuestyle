...
const CartPopup = ({ onClose }) => {
  ...
  // Cookie se cart read karo
  useEffect(() => {
    try {
      const existing = JSON.parse(Cookies.get(CART_KEY) || "[]");
      setItems(existing);
    } catch (e) {
      console.log("guest cart parse error:", e);
      setItems([]);
    }
  }, []);

  // Total items & price (future me checkout ke liye kaam ayega)
  ...
  const redirect = () => {
    navigate("/checkout", {
      state: {
        products: items,         // cookie se products
        price: totalPrice,       // total amount
        shipping_fee: shippingFee,         // agar shipping fee alag se hai to yahan update kar lena
        items: totalItems,
      },
    });
  };
